(function () {
  ...
  function responsiveImage(image, alt, options = {}) {
      ...
      .map(
        (item) => `
          <article class="product-card surface product-card--brand">
            <div class="product-card__media">
              ${responsiveImage(item.image, item.alt, { sizes: "(max-width: 48rem) calc(100vw - 2rem), (max-width: 70rem) 50vw, 20vw" })}
            </div>
            <div class="product-card__body">
              <div class="brand-kicker">${item.brand}</div>
              <div class="home-brand-grid__intro">
                <h3><a href="${item.href}">${item.ingredient}</a></h3>
                <p>${item.summary}</p>
              </div>
              <div class="tag-list home-brand-grid__markers">
                ${item.markers.map((marker) => `<span class="tag">${marker}</span>`).join("")}
              </div>
              <div class="home-brand-grid__applications">
  ...
  async function renderProductDetail() {
    ...
    mediaNode.innerHTML = responsiveImage(product.image, product.alt, {
      loading: "eager",
      priority: true,
      sizes: "(max-width: 64rem) calc(100vw - 2rem), 38vw",
    });
