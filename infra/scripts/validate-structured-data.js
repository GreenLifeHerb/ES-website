...
function validateSitemapAndRobots() {
  ...
  assert(fs.existsSync(llmsPath), "llms.txt does not exist");
  ...
  assert(
    /Sitemap:\s*https:\/\/essencesourceusa\.com\/sitemap-index\.xml/i.test(robots),
    "robots.txt missing sitemap index URL",
  );
  assert(/https:\/\/essencesourceusa\.com\/llms\.txt/i.test(robots), "robots.txt missing llms.txt hint");
  assert(
    llms.includes("https://essencesourceusa.com/coa-tds-request-checklist.html"),
    "llms.txt missing COA/TDS guide URL",
  );
