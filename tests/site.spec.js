...
test("product filters reduce the visible list", async ({ page }) => {
  ...
  await expect(page.locator("[data-product-card]")).toHaveCount(49);
...
test("contact form prefilled request type updates buyer helper", async ({
  page,
}) => {
  await page.goto("/contact.html?inquiry_type=docs#contact-form", {
    waitUntil: "domcontentloaded",
  });
  await expect(page.locator("[data-inquiry-helper-title]")).toContainText(
    "For COA/TDS requests",
  );
  await page.selectOption("#field-inquiry_type", "sample");
  await expect(page.locator("[data-inquiry-helper-title]")).toContainText(
    "For sample requests",
  );
});
...
test("product detail shows specification table and inquiry button", async ({
  ...
  await expect(
    page
      ...
      .getByRole("link", { name: /Request Quote/i }),
  ).toBeVisible();
});
