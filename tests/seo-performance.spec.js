...
test("image-sitemap.xml exists and includes product images", async () => {
  const sitemap = read("image-sitemap.xml");
  expect(sitemap).toContain("xmlns:image");
  expect(sitemap).toContain("product-green-coffee.html");
  expect(sitemap).toContain("photo-green-coffee-extract-1200.webp");
});

test("sitemap-index.xml references standard and image sitemaps", async () => {
  const sitemapIndex = read("sitemap-index.xml");
  expect(sitemapIndex).toContain("https://essencesourceusa.com/sitemap.xml");
  expect(sitemapIndex).toContain("https://essencesourceusa.com/image-sitemap.xml");
});

test("robots.txt declares sitemap and blocks backend paths", async () => {
  const robots = read("robots.txt");
  expect(robots).toContain("Sitemap: https://essencesourceusa.com/sitemap-index.xml");
  expect(robots).toContain("https://essencesourceusa.com/llms.txt");
  expect(robots).toMatch(/Disallow:\s*\/admin/);
  expect(robots).toMatch(/Disallow:\s*\/api\/internal/);
});

test("llms.txt describes buyer-critical pages", async () => {
  const llms = read("llms.txt");
  expect(llms).toContain("Essence Source is a U.S.-market B2B botanical ingredient sourcing website");
  expect(llms).toContain("https://essencesourceusa.com/coa-tds-request-checklist.html");
  expect(llms).toContain("https://essencesourceusa.com/green-coffee-bean-extract-supplier-guide.html");
});
